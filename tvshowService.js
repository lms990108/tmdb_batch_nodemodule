...
async function fetchTVShowIds(year, page) {
  try {
    const url = `https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}&include_adult=false&include_video=false&language=ko-KR&page=${page}&sort_by=first_air_date.asc&first_air_date_year=${year}`;
    const response = await axios.get(url);
    return response.data.results.map((tvShow) => tvShow.id);
  } catch (error) {
    logger.error(
      `Error fetching TV show IDs for year ${year}, page ${page}: ${error.message}`
    );
    return [];
  }
}

async function fetchAndStoreTVShowDetails(tvShowId, connection) {
  try {
    ...
    await connection.execute(insertTVShowQuery, [
      tvShow.id,
      tvShow.adult,
      tvShow.backdrop_path ? TMDB_IMAGE_BASE_URL + tvShow.backdrop_path : null,
      firstAirDate,
      lastAirDate,
      tvShow.name,
    ...
    for (const genre of genres) {
      ...
      await connection.execute(insertTVShowGenreQuery, [tvShow.id, genre.id]);
    }
    ...
    for (const actor of cast) {
      ...
      await connection.execute(insertActorQuery, [
        actor.id,
        actor.name,
        actor.profile_path ? TMDB_IMAGE_BASE_URL + actor.profile_path : null,
      ]);
      ...
      await connection.execute(insertTVShowActorQuery, [tvShow.id, actor.id]);
    }
    ...
    if (error.response && error.response.status === 404) {
      logger.error(`TV Show ID ${tvShowId} not found: ${error.message}`);
    } else {
...
async function processTVShowsByYear(year, connection) {
  let page = 1;
  while (true) {
    const tvShowIds = await fetchTVShowIds(year, page);
    if (tvShowIds.length === 0) {
      break;
    }

    for (const tvShowId of tvShowIds) {
      await fetchAndStoreTVShowDetails(tvShowId, connection);
    }
